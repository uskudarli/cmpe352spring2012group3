...
public class EventDriver {
	...
	public static boolean createEvent(int userId,String description,String place,java.sql.Timestamp eventTime) {
		try{
...
public static EventTable getEvent(int EventId) {
	try {
		...
		ps.setInt(1, EventId);
		ResultSet result = ps.executeQuery();
		return new EventTable(result.getInt("id"),result.getInt("user_id"),result.getString("description"),result.getString("place"),result.getTimestamp("event_time"));
	} catch(SQLException e) {
		System.out.println(e.getMessage());
		return null;
	} catch (Exception e) {
...
private static EventTable[] convertEventToArray(ResultSet result) throws SQLException{
	...
	int i=0;
	while(result.next())
		events[i++] = new EventTable(result.getInt("id"),result.getInt("user_id"),result.getString("description"),result.getString("place"),result.getTimestamp("event_time"));
	return events;
}
